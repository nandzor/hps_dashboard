...
@section('page-title', 'Create WhatsApp Settings')

@section('content')
  <div class="max-w-4xl">
    <x-card title="Create WhatsApp Settings">
      <div class="mb-6">
        <p class="text-sm text-gray-500">Configure WhatsApp phone numbers and message template for notifications</p>
      </div>

      <form method="POST" action="{{ route('whatsapp-settings.store') }}" class="space-y-6">
        @csrf

        <!-- Basic Information -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <x-input name="name" label="Settings Name" placeholder="e.g., Default, Emergency, Admin" required
            hint="Unique name for this WhatsApp settings" />

          <x-input name="description" label="Description" placeholder="Brief description of this settings"
            hint="Optional description for this WhatsApp settings" />
        </div>

        <!-- Phone Numbers -->
        <div>
          <x-textarea name="phone_numbers" label="Phone Numbers" rows="3"
            placeholder="081234567890, 081234567891, 081234567892" required
        ...
        <div>
          <x-textarea name="message_template" label="Message Template" rows="6"
            placeholder="Enter your message template here..." required
        ...
        <div class="border-t border-gray-200 pt-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-4">
              <x-checkbox name="is_active" label="Active Settings" :checked="true"
                hint="Enable or disable this WhatsApp settings" />
            </div>

            <div class="space-y-4">
              <x-checkbox name="is_default" label="Set as Default" :checked="false"
